...
export type CloudTaskState = "queued" | "leased" | "running" | "waiting_input" | "waiting_permission" | "stalled" | "cancel_requested" | "succeeded" | "failed" | "cancelled" | "unknown";
...
export class CloudTaskStore implements NotificationOutbox {
  ...
  requestCancellation(taskId: string): CloudTask {
    ...
    } else if (current.state === "leased" || current.state === "running" || current.state === "waiting_input" || current.state === "waiting_permission" || current.state === "stalled" || current.state === "cancel_requested") {
  ...
  heartbeat(workerId: string, metadata?: Record<string, unknown>, leaseMs = 90_000): CloudWorker {
    ...
    this.db.prepare(`UPDATE cloud_tasks SET lease_expires_at = ?, updated_at = ?
      WHERE lease_owner = ? AND state IN ('leased', 'running', 'waiting_input', 'waiting_permission', 'stalled', 'cancel_requested')`)
  ...
  activeForWorker(workerId: string): CloudTask[] {
    const rows = this.db.prepare(`SELECT * FROM cloud_tasks
      WHERE worker_id = ? AND lease_owner = ? AND state IN ('leased', 'running', 'waiting_input', 'waiting_permission', 'stalled', 'cancel_requested')
    ...
    const transaction = this.db.transaction(() => {
      ...
      const candidate = candidates.find((item) => {
        ...
        const active = this.db.prepare(`SELECT COUNT(*) AS count FROM cloud_tasks
          WHERE project_id = ? AND state IN ('leased', 'running', 'waiting_input', 'waiting_permission', 'stalled', 'cancel_requested')`).get(item.project_id) as { count: number };
  ...
  updateFromWorker(taskId: string, workerId: string, state: Exclude<CloudTaskState, "queued" | "leased">, result?: Record<string, unknown>, leaseMs = 90_000): CloudTask {
    ...
    const nextState = current.state === "cancel_requested" && (state === "running" || state === "waiting_input" || state === "waiting_permission" || state === "stalled") ? "cancel_requested" : state;
...
function allowsWorkerTransition(current: CloudTaskState, next: CloudTaskState): boolean {
  const active = next === "running" || next === "waiting_input" || next === "waiting_permission" || next === "stalled";
  if (current === "leased") return active || next === "succeeded" || next === "failed" || next === "cancelled" || next === "unknown";
  if (current === "running" || current === "waiting_input" || current === "waiting_permission" || current === "stalled") {
