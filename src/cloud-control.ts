...
const workerUpdate = z.object({
  state: z.enum(["running", "waiting_input", "waiting_permission", "stalled", "succeeded", "failed", "cancelled", "unknown"]),
...
const listInput = z.object({ projectId: z.string().min(1).max(80).optional(), state: z.enum(["queued", "leased", "running", "waiting_input", "waiting_permission", "stalled", "cancel_requested", "succeeded", "failed", "cancelled", "unknown"]).optional(), limit: z.coerce.number().int().min(1).max(100).optional() });
