...
- M2：实施中。项目受限的 CodeBuddy 派发、状态刷新、普通追问和取消已实现；远端准备器会检查仓库干净、固定 base SHA 并创建受控 worktree/分支。断线回执关联、工作区恢复和真实端到端验证仍未完成，不能声称达到 M2 完成标准。
- M3：实施中。远端受限 Git 核验及 GitHub CLI 草稿 PR 创建正在实现。尚未完成真实测试仓库的端到端验证。
- M4：实施中。SQLite 持久事件 outbox、MCP 查询/确认接口、独立 coordinator 轮询命令和可选 Webhook 投递已实现；系统服务安装仍未实现。
